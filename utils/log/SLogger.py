...
use:
  logging = LogConfig(log_config_file, log_data_file=None)
  logger = logging.getLogger(self.log_key)
or:
  logger = SLogger(log_config_file, log_data_file=None)

to:
  1, specify the logging data saving directory!
...
class LogConfig(object):
  ...
  def _parse_option(self):
    ...
    for handler, handler_info in self.handlers.items():
      ...
      handler_args = list(eval(self.log_config.get(section_name, "args")))
      ...
      if (isinstance(_handler, logging.FileHandler) or issubclass(_handler, logging.FileHandler)):
  ...
  def __parse_logger(self, logger_name, section_name):
    ...
    for k, v in tuple_items:
      if k == "handlers":
        handlers = filter(lambda x:x, [h.strip() for h in v.split(",")])
        for h in handlers:
...
class SLogger(object):
  ...
  def debug(self, key, debug_info):
    self.logger.debug("[{0}]: {1}".format(key, debug_info))
  def critical(self, key, info):
    self.logger.critical("[{0}]: {1}".format(key, info))
...
def test():
  ...
  log_data_file = "./log/log/run.log"
  print "{0}:{1}".format(log_config_file, os.path.exists(log_config_file))
  t = SLogger(log_config_file, log_key, log_data_file)
  t.info("TEST TRACE", "OK")
  t.error("TEST TRACE", "OK")
  t.debug("TEST TRACE", "OK")
  t.critical("TEST TRACE", "OK")
