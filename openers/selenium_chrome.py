...
if __name__ == "__main__":
    ...
    get_sleep_time = lambda : 1
else:
    from default_settings import CHROME_PATH
    from tools import get_sleep_time


class SelChromeOpener(Opener):
    def __init__(self, timeout=60, debug=False, **kwargs):
        super(SelChromeOpener, self).__init__(timeout=timeout, debug=debug, **kwargs)
        self.driver = None

    def initialize(self):
        self.driver = webdriver.Firefox(executable_path=CHROME_PATH)
        time.sleep(10)
        print("initializing 10s and continue ..")
